...
export default function Home() {
  ...
  const { data: mixes } = useQuery({
    queryKey: ['mixes'],
    queryFn: async () => {
      const res = await eb.db.from('custom_mixes');
      ...
      {themes?.map((t) => (
        <ThemeCard
          ...
          onPress={() => {
            ...
            else router.push({ pathname: '/session/ready', params: { themeId: t.id } });
          }}
        />
      ))}
      {mixes && mixes.length > 0 && (
        <>
          ...
          {mixes.map((m: { id: string; name: string }) => (
            <Pressable
              key={m.id}
              onPress={() => router.push({ pathname: '/session/ready', params: { mixId: m.id } })}
